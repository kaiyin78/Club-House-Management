...
public class LinkedList <T> implements LinkedListInterface<T>,Serializable{
    ...
    public LinkedList(int initialCapacity) {
        size = 0;
        staffArray = (T[]) new Object[initialCapacity];
    }
    ...
    /*
        ...
        if (givenPosition <= size) {
            result = promotionArray[givenPosition];
        }
//        
...
//        }
  ...
  public int count(int givenPosition){
      int max = 0;
      if(!isEmpty()){
          for(int i = frontIndex; i <= backIndex; i++){
              if(max < frontIndex){
                  max = (int) staffArray[frontIndex];
              }
          }
      }
      return max;
